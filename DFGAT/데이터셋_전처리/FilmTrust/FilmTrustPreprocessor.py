...
class FilTrustPreprocessorNoRating:
    def __init__(self):
        # [uid, mid, timestamp] 형태로 저장할 리스트 (레이팅 없음)
        self.edge_datas = []
    ...
    def load_edge_data(self):
        ...
        [uid, mid, timestamp] 형태로 self.edge_datas에 저장.
        rating은 불러오되, 최종적으로 저장/활용하지 않음.

        파일 예시 (헤더 없음):
        1 1 2
        1 2 4
        ...
        각 줄은 (userID, movieID, rating) 순서로 기록됨.
        """
        with open(self.ratings_path, 'r', encoding='utf-8') as fp:
            ...
            for i, line in enumerate(lines):
                ...
                user = user_str.strip()   
                movie = movie_str.strip()  
                # rating = float(rating_str)  # <- 실제 코드는 사용 안 함

                # i번째 줄을 timestamp(가짜)로 지정
                timestamp = i

                # [uid, mid, timestamp]만 저장
                self.edge_datas.append([user, movie, timestamp])
    ...
    def split_save_pkl(self):
        """
        - 유저별로 [timestamp]를 기준으로 정렬
        - 60% / 20% / 20% 비율로 (train, val, test) 분할
        - 유저별 데이터가 5개 이하인 경우 무시
        - pkl 파일로 각각 저장
        - 저장 형식 [uid, mid, timestamp]
        """
        TRAIN_RATIO = 0.6
        VAL_RATIO   = 0.2
        TEST_RATIO  = 0.2
        ...
        # uid별로 데이터를 그룹화: uid2edges[uid] = [[timestamp, mid], [timestamp, mid], ...]
        uid2edges = defaultdict(list)

        for edge_data in self.edge_datas:
            # edge_data = [uid, mid, timestamp]
            uid, mid, timestamp = edge_data
            uid2edges[uid].append([timestamp, mid])
        
        # timestamp 기준 정렬
        for uid in uid2edges:
            # uid2edges[uid] = [[timestamp, mid], ...]
            uid2edges[uid].sort(key=lambda x: x[0])  # x[0] = timestamp
        ...
        for uid, record_list in uid2edges.items():
            if len(record_list) <= 5:
                print(f"유저 {uid}는 데이터가 5개 이하이므로 스킵합니다.")
                continue
            ...
            # record_list[i] = [timestamp, mid]
            for (timestamp, mid) in record_list[:train_end]:
                train_edges.append([uid, mid, timestamp])
            for (timestamp, mid) in record_list[train_end:val_end]:
                val_edges.append([uid, mid, timestamp])
            for (timestamp, mid) in record_list[val_end:]:
    ...
    def preprocess(self):
        """
        전체 전처리 단계 순서:
        1) ratings.txt 로드 -> edge_datas ([uid, mid, timestamp])
        ...
        print("=== 전처리 완료! 아래는 마지막 5개 [uid, mid, timestamp] 데이터 ===")
        for row in self.edge_datas[-5:]:
            print(row)


if __name__ == "__main__":
    pr = FilTrustPreprocessorNoRating()
    pr.preprocess()
