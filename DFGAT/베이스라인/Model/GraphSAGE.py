...
class TwoLayerSimpleHeteroSAGE(nn.Module):
    def __init__(self, num_user_nodes, num_location_nodes, emb_dim, out1_dim, out2_dim, dataset):
        ...
        # SAGEConv 사용, aggregator_type='mean' (다른 방법: 'pool', 'lstm', etc. 가능)
        self.conv1 = HeteroGraphConv({
            'go': SAGEConv(in_feats=self.emb_dim, out_feats=self.out1_dim, aggregator_type='mean'),
            'back': SAGEConv(in_feats=self.emb_dim, out_feats=self.out1_dim, aggregator_type='mean'),
        }, aggregate='mean')
        ...
        self.conv2 = HeteroGraphConv({
            'go': SAGEConv(in_feats=self.out1_dim, out_feats=self.out2_dim, aggregator_type='mean'),
            'back': SAGEConv(in_feats=self.out1_dim, out_feats=self.out2_dim, aggregator_type='mean'),
        }, aggregate='mean')
